...
fn test_create_goal_with_existing_area() {
    ...
    let goal1 = goal_service
        .create_goal("完成项目A", "工作", "描述A", GoalStatus::Active)
        .unwrap();
    assert!(goal1.area_id.is_some());

    let goal2 = goal_service
        .create_goal("完成项目B", "工作", "描述B", GoalStatus::Active)
        .unwrap();
    assert_eq!(
...
fn test_create_goal_with_new_area_auto_creates_area() {
    ...
    let goal = goal_service
        .create_goal("学习 Rust", "学习", "深入学习", GoalStatus::Active)
        .unwrap();
...
fn test_create_goal_with_empty_area_defaults_to_uncategorized() {
    ...
    let goal = goal_service
        .create_goal("临时任务", "", "", GoalStatus::Active)
        .unwrap();
    assert!(goal.area_id.is_some());
...
fn test_create_goal_with_whitespace_area_defaults_to_uncategorized() {
    ...
    let goal = goal_service
        .create_goal("另一个临时任务", "   ", "", GoalStatus::Active)
        .unwrap();
...
fn test_all_goals_have_non_null_area_id() {
    ...
    goal_service
        .create_goal("Goal 1", "工作", "", GoalStatus::Active)
        .unwrap();
    goal_service
        .create_goal("Goal 2", "", "", GoalStatus::Active)
        .unwrap();
    goal_service
        .create_goal("Goal 3", "学习", "", GoalStatus::Active)
        .unwrap();
    ...
    for summary in &summaries {
        ...
        assert!(
            !summary.area.is_empty(),
            "Goal '{}' should have an area",
            summary.title
        );
    }
}
