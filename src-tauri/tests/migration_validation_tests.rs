...
fn test_migration_from_legacy_database() {
    ...
    // 应该有 3 个 areas
    let area_count: i64 = connection.query_row("SELECT COUNT(*) FROM areas", [], |row| row.get(0))
        .expect("Failed to count areas");
    assert_eq!(area_count, 3, "应该有 3 个原始 areas");

    // 应该有 8 个 goals
    let goal_count: i64 = connection.query_row("SELECT COUNT(*) FROM goals", [], |row| row.get(0))
    ...
    let area_count_after: i64 = connection.query_row("SELECT COUNT(*) FROM areas", [], |row| row.get(0))
        .expect("Failed to count areas after migration");
    assert_eq!(area_count_after, 4, "迁移后应该有 4 个 areas（包括'未分类'）");

    // "未分类" area 应该存在
    let uncategorized_exists: bool = connection.query_row(
    ...
    let uncategorized_goal_count: i64 = connection.query_row(
        "SELECT COUNT(*) FROM goals WHERE area_id = ?1",
        [UNCATEGORIZED_AREA_ID],
        |row| row.get(0)
    ).expect("Failed to count uncategorized goals");
    assert_eq!(uncategorized_goal_count, 4, "'未分类'领域应该有 4 个 goals（原孤儿 goals）");

    // 正常关联的 goals 的 area_id 不应该改变
    let work_goal_count: i64 = connection.query_row(
    ...
    let life_goal_count: i64 = connection.query_row(
    ...
    let learning_goal_count: i64 = connection.query_row(
...
fn test_migration_idempotency() {
    ...
    // 第一次迁移
    repository.initialize().expect("First migration should succeed");

    let connection = Connection::open(&db_path).expect("Failed to open database");
    let area_count_first: i64 = connection.query_row("SELECT COUNT(*) FROM areas", [], |row| row.get(0))
        .expect("Failed to count areas");
    let goal_count_first: i64 = connection.query_row("SELECT COUNT(*) FROM goals", [], |row| row.get(0))
        .expect("Failed to count goals");
    let uncategorized_goal_count_first: i64 = connection.query_row(
        "SELECT COUNT(*) FROM goals WHERE area_id = ?1",
        [UNCATEGORIZED_AREA_ID],
        |row| row.get(0)
    ).expect("Failed to count uncategorized goals");
    ...
    // 第二次迁移
    repository.initialize().expect("Second migration should succeed");

    let connection = Connection::open(&db_path).expect("Failed to open database");
    let area_count_second: i64 = connection.query_row("SELECT COUNT(*) FROM areas", [], |row| row.get(0))
        .expect("Failed to count areas");
    let goal_count_second: i64 = connection.query_row("SELECT COUNT(*) FROM goals", [], |row| row.get(0))
        .expect("Failed to count goals");
    let uncategorized_goal_count_second: i64 = connection.query_row(
        "SELECT COUNT(*) FROM goals WHERE area_id = ?1",
        [UNCATEGORIZED_AREA_ID],
        |row| row.get(0)
    ).expect("Failed to count uncategorized goals");
    ...
    // 验证数据没有重复
    assert_eq!(area_count_first, area_count_second, "重复迁移不应该创建重复的 areas");
    assert_eq!(goal_count_first, goal_count_second, "重复迁移不应该改变 goals 数量");
    assert_eq!(uncategorized_goal_count_first, uncategorized_goal_count_second, "重复迁移不应该重复移动 goals");

    // "未分类" area 应该只有一个
    let connection = Connection::open(&db_path).expect("Failed to open database");
    let uncategorized_count: i64 = connection.query_row(
    ...
    // 第三次迁移，确保完全幂等
    drop(connection);
    repository.initialize().expect("Third migration should succeed");

    let connection = Connection::open(&db_path).expect("Failed to open database");
    let area_count_third: i64 = connection.query_row("SELECT COUNT(*) FROM areas", [], |row| row.get(0))
        .expect("Failed to count areas");
...
fn test_clean_database_migration() {
    ...
    let repository = SqliteRepository::new(db_path.clone());
    repository.initialize().expect("Migration on clean database should succeed");
...
fn test_list_areas_returns_is_system_field() {
    ...
    // 加载所有 areas
    let workspace = repository.load_workspace().expect("Failed to load workspace");

    // 应该有 2 个 areas
    assert_eq!(workspace.areas.len(), 2, "应该有 2 个 areas（'未分类' + '测试领域'）");

    // 找到"未分类" area 并验证 is_system = true
    let uncategorized = workspace.areas.iter()
        .find(|a| a.id.to_string() == UNCATEGORIZED_AREA_ID)
        .expect("应该找到'未分类' area");
    assert!(uncategorized.is_system, "'未分类' area 的 is_system 应该为 true");
    assert_eq!(uncategorized.title, "未分类", "'未分类' area 的标题应该正确");

    // 找到普通 area 并验证 is_system = false
    let normal_area = workspace.areas.iter()
        .find(|a| a.id.to_string() == test_area_id)
        .expect("应该找到'测试领域' area");
    assert!(!normal_area.is_system, "普通 area 的 is_system 应该为 false");
