...
- 任务详情抽屉已经完成一轮可用性升级：
  ...
  - “快速创建目标并关联”已并入目标选择面板
- 新的 RecallLoom `.recallloom/` 已由官方 `init_context.py` 重建并通过 `validate_context.py`
- 旧的损坏 sidecar 已保留在 `.recallloom.damaged-2026-06-11/` 作为恢复备份
...
- RecallLoom 修复优先走官方 helper，而不是继续手补旧 sidecar 的损坏结构。
...
- 仓库里的 `@tauri-apps/cli` 仍存在 optional native binding 缺失问题，直接走 `npm run tauri:dev`/默认 Playwright `webServer` 可能失败。
...
- 当前任务从“仅恢复上下文”升级为“完成 UI 改版并修复连续性系统”。
