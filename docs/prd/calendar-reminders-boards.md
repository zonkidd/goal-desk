...
- **按清单视图**：自适应网格布局（2-4列），每个 Reminder List 显示为独立的卡片面板，清单内的提醒以只读状态列表形式展示
- **按时间视图**：按紧急度分组（已过期 / 今天 / 未来7天 / 更晚 / 无日期），可折叠，不同颜色区分优先级

两种视图通过顶部 Tab 切换，支持全局"隐藏已完成"开关，点击提醒项打开只读详情抽屉；完成状态来自系统导入，不在 Goal Desk 中修改。
...
2. 作为一个使用 macOS 日历和提醒的用户，我希望在 Goal Desk 中直接查看这些系统数据，避免频繁切换应用
...
9. 作为一个完成提醒后的用户，我希望 Goal Desk 只读展示系统提醒的完成状态，并在需要修改时打开系统提醒事项
...
- `openSystemReminder(id)` - 打开系统提醒事项 App
...
返回指定日期范围的 Calendar Events 和 Reminders。前端通过 EventKit range 模块在周视图初始化时加载当前周 ± 1 周的数据（共 3 周）：当前范围缓存在 hook 状态用于看板渲染，同时按 EventKit id 以最新数据为准合并进全局 EventKit store，确保点击范围加载的 Calendar Event / System Reminder 时详情抽屉能解析到同一条只读外部源数据。
...
点击提醒时：
1. 打开只读详情抽屉或系统提醒事项 App
2. 如果"隐藏已完成"开关打开，已完成的提醒从列表中隐藏
3. 已完成提醒根据系统导入的 `done` 状态添加 `opacity-60` 和划线样式
...
- 只读提醒：点击提醒时，验证打开详情抽屉或系统提醒事项 App
...
- 导航到提醒看板 → 切换按清单/按时间视图 → 点击提醒打开只读详情抽屉
...
2. **系统提醒写入** - 当前只支持查看和打开系统提醒事项，不支持在 Goal Desk 中创建、编辑或标记完成系统提醒
3. **拖拽调整事件时间** - 周视图中拖动事件卡片到其他日期，需要额外的拖拽库和本地 Todo 更新时间逻辑；Calendar Event 和 Reminder 仍保持只读
