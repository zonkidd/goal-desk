...
export function Sidebar() {
  ...
  return (
    <aside className="glass-panel relative z-20 flex h-full w-[260px] shrink-0 flex-col justify-between border-r border-white/15">
      ...
      <nav data-testid="sidebar-scroll" className="nav-container flex-1 space-y-1.5 overflow-y-auto overscroll-contain px-4 pt-4 pb-0">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative nav-btn flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors",
                active
                  ? "text-theme-accent"
            ...
            >
              {active && (
                <motion.div
                  ...
                  className="absolute inset-0 rounded-xl border border-white/10 bg-theme-card shadow-[0_0_15px_rgba(6,182,212,0.15)]"
              ...
              {item.key === "inbox" && (
                <span className="relative z-10 rounded-full bg-theme-accent-light px-2 py-0.5 text-xs font-bold text-theme-accent transition-colors">
                  {inboxCount}
                </span>
              )}
            </button>
      ...
      <div data-testid="sidebar-footer-gap" className="h-5 shrink-0" aria-hidden />
      <div data-testid="sidebar-footer" className="mx-4 mb-6 flex shrink-0 items-center gap-2">
        <button
