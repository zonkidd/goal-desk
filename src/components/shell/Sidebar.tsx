...
export function Sidebar() {
  ...
  return (
    <aside className="glass-panel relative z-20 flex h-full w-[260px] shrink-0 flex-col justify-between border-r border-white/50">
      ...
      <nav className="nav-container flex-1 space-y-1.5 overflow-y-auto px-4 py-4">
        ...
        <motion.button
          ...
          className={cn(
            ...
            activeArea === "ALL"
              ? "bg-white/70 text-indigo-600 shadow-sm ring-1 ring-indigo-200"
              : "text-slate-600"
          )}
        >
        ...
        {allAreas.map((area, index) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`h-2 w-2 rounded-full ${swatch}`} />{" "}
                {area.title}
              </div>
            </motion.button>
          );
      ...
      >
        <span>全局速记</span>
        <kbd className="rounded-md border border-slate-200 bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] text-slate-400">
          ⌥ Space
        </kbd>
      </button>
    </aside>
